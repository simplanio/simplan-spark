...
class KafkaBatchSource(sparkAppContext: SparkAppContext, operatorContext: OperatorContext) extends SparkOperator[KafkaBatchSourceConfig](sparkAppContext, operatorContext) {
  ...
  override def process(request: SparkOperatorRequest): SparkOperatorResponse = {
    ...
    val loadedDataframe = sparkAppContext.spark.read
      .format("kafka")
