...
/** @author Abraham, Thomas - tabraham1
  *         Created on 04-Sep-2022 at 10:09 AM
  */
class DeltaMergeForEachBatchHandler(sparkAppContext: SparkAppContext) extends ForEachBatchHandler(sparkAppContext) {

  override def handle(request: SparkOperatorRequest, handlerConfigJson: String): (DataFrame, Long) => Unit = {
    implicit val handlerConfig: DeltaMergeConfig = handlerConfigJson.fromJson[DeltaMergeConfig]
    implicit val deltaTable: DeltaTable = request.deltaTable(handlerConfig.deltaTableSource)
    DeltaMergeUtils.forEachBatchDataframeWriter
  }
}
